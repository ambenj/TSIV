...
blastn_file <- "../../analysis/assembly/03_annotate/geneMarkS_STMY_X_2011_03_M_all/STMY_X_2011_03_M_all_GeneMarkS_blastp.tsv"
```
...
gff %>% 
  mutate(NODE_temp = str_remove(seqid, "NODE_")) %>% 
...
```{r}
# How many genes are annotated for NODE_71?
gff %>% 
    filter(grepl('NODE_71_', seqid))

# What blast hits does that gene_134 have?
blastn %>% 
  filter(grepl('gene_134\\|', qseqid)) %>% 
  arrange(-bitscore)
...
```{r}
# Filter to just TSIV contigs
TSIV_gff <- gff %>% 
  filter(grepl('NODE_1_|NODE_2_|NODE_3_|NODE_18_|NODE_71', seqid))

# Filter to just blastn results on TSIV contigs
TSIV_blastn <- inner_join(blastn, TSIV_gff, by = join_by(gene_id))
...
```{r}
# Get TSIV blast hits from Frog Virus 3 (FV3; taxID: 10493)
TSIV_blastn %>%
  filter(staxid == 10493) %>% 
  group_by(gene_id) %>% 
  summarise(n=n()) %>% 
